...
const TagStringIcon: React.FC<NormalTableComponentProps> = (props) => {
  ...
  return (
    <span className='frc-table-bid-box'>
      {tagArr &&
        (tagArr as any).map((item: { text: string; color: string }) => {
          return (
            <span
      ...
      <span style={{ width: 14, display: 'inline-block' }}>
        {iconType ? (
          <Tooltip title={(iconType as any) === 'need-to-ask' ? '需请示' : '必须请示'} hasArrow={false}>
            <Icon type={iconType as any} style={{ color: '#097c6d' }} />
          </Tooltip>
        ) : (
          ''
        )}
